...
const processData = (data: DataType[]): ProcessedDataType => {
    ...
    data.reverse().forEach((dataPoint) => {
        ...
        dataPoint.samples.forEach((sample) => {
            // const timeValue = dataPoint.sample_interval * index * 1000; // Convert to milliseconds
            // time_vector.push(timeValue);
            ...
            try {
                ...
                setVisibleData((prevVisibleData) => {
                    ...
                    Object.keys(processedData).forEach((key) => {
                        ...
                        const newMeasurementData = processedData[dataKey]?.measurement_data || [];
                        const sampleInterval = processedData[dataKey]?.sample_interval || 1.024462; // Ensure sample_interval is used
                        ...
                        if (prevTimeVector.length > 0 && newTimeVector.length > 0) {
                            const lastPreviousTime = prevTimeVector[prevTimeVector.length - 1];
                            ...
                            if (absoluteFirstNewTime <= absoluteLastPreviousTime) {
                                console.warn(`Discontinuity detected for ${dataKey}! Last previous absolute time: ${absoluteLastPreviousTime}, First new absolute time: ${absoluteFirstNewTime}`);
                            } else {
                                console.log(`Data for ${dataKey} is continuous. Last previous absolute time: ${absoluteLastPreviousTime}, First new absolute time: ${absoluteFirstNewTime}`);
                            }
                        }
